...
The framework is applied to Nepal's catastrophic 2014 Jure landslide. HYDRUS-1D serves as a teacher model, simulating 1D vertical unsaturated infiltration governed by the Richards equation to generate physically consistent pseudo-data points of pressure head $\psi(z,t)$. The PINN is trained on a 14-day critical window (334 hours) using a physics-aware loss function that enforces the Richards equation via automatic differentiation, satisfies boundary conditions, and aligns with 1,000 anchor points and 18,412 failure points using Adam and L-BFGS optimisation. The Factor of Safety is computed using the infinite-slope stability model with Van Genuchten--Mualem hydraulic properties.
...
Sindhupalchok, Nepal. The framework models 1D vertical unsaturated infiltration over a 55 m soil column using HYDRUS-1D-generated pseudo-data (1,000 anchor points across a 14-day / 334-hour window) as anchor data, directly addressing the data-scarcity challenge of unmonitored Himalayan slopes.
...
\multirow{6}{*}{\textbf{Hydraulic}}
    & Saturated Hydraulic Conductivity & $K_s$       & $5.56\times10^{-6}$ m/s \\
    & Saturated Water Content          & $\theta_s$  & 0.45 \\
    & Residual Water Content           & $\theta_r$  & 0.06 \\
    & Van Genuchten Alpha              & $\alpha$    & 1.8 m$^{-1}$ \\
    & Pore-size Distribution Index     & $n$         & 1.4 \\
    & Tortuosity / Connectivity        & $l$         & 0.5 \\
\hline
\multirow{3}{*}{\textbf{Geotechnical}}
    & Effective Cohesion               & $c'$        & 115.0 kPa \\
    & Effective Friction Angle         & $\phi'$     & 34.5° \\
    & Bulk Unit Weight                 & $\gamma$    & 18.5 kN/m$^3$ \\
\hline
...
where $z_{\max} = 55.0$ m, $t_{\max} = 334.0$ hours,
$\psi_{\min} = -2.540$ m, and $\psi_{\max} = 1.737$ m, derived directly from the HYDRUS training data.
...
\textbf{Phase I — Adam with StepLR (5,000 epochs):}
...
\begin{itemize}
    ...
    \item \textbf{Hardware:} MPS (Metal Performance Shaders) on Apple Silicon; GPU acceleration was used for training, while only CPU is required for inference after training.
