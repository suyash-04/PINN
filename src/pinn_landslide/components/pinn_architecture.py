...
class PINN(nn.Module):
    def __init__(self, arch_config: PINNArchConfig):
        super().__init__()
    ...
    def build_network(self) -> nn.Sequential:  
        # Get the actual PyTorch class reference (e.g., nn.Tanh)
        act_class = self._get_activation() 
        layers = []
        
        # Input layer
        layers.append(nn.Linear(self.arch_config.input_dim, self.arch_config.neurons_per_layer))
        layers.append(act_class()) # Now it correctly calls nn.Tanh()
        ...
        for _ in range(self.arch_config.n_hidden_layers):
            ...
            layers.append(act_class()) # Correctly calls nn.Tanh()

        # Output layer (Note: self.arch_config.output_dim instead of just self.output_dim)
        layers.append(nn.Linear(self.arch_config.neurons_per_layer, self.arch_config.output_dim))
