...
## Saving and Loading Models / Batches

Model checkpointing and tensor serialization use PyTorch primitives.

Save a model state dict (recommended):

```python
import torch
torch.save(model.state_dict(), 'artifacts/model/pinn_model_state.pt')
```
...
```python
model = PINN(...)
model.load_state_dict(torch.load('artifacts/model/pinn_model_state.pt'))
model.eval()
```
...
torch.save({
    ...
    'model_state_dict': model.state_dict(),
    'optimizer_state_dict': optimizer.state_dict()
}, 'artifacts/model/pinn_checkpoint.pt')

checkpoint = torch.load('artifacts/model/pinn_checkpoint.pt')
model.load_state_dict(checkpoint['model_state_dict'])
optimizer.load_state_dict(checkpoint['optimizer_state_dict'])
start_epoch = checkpoint['epoch']
```

Batch tensors are saved/loaded similarly:

```python
# save batched tensors
torch.save({'inputs': inputs_tensor, 'targets': targets_tensor}, 'artifacts/dataset/data_batch.pt')

# load batched tensors
batch = torch.load('artifacts/dataset/data_batch.pt')
inputs = batch['inputs']
